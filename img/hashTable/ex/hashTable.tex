...
\node[listnode,right=of t0] (carol) {4 $\mapsto$ Carol};
\node[listnode,right=.5cm of carol] (eve) {12 $\mapsto$ Eve};
\node[listnode,right=of t1] (alice) {1 $\mapsto$ Alice};
\node[listnode,right=.5cm of alice] (dan) {9 $\mapsto$ Dan};
...
\node[right=.5cm of eve] (evex) {$\varnothing$};
\node[right=.5cm of dan] (danx) {$\varnothing$};
\node[right=of t2] (t2x) {$\varnothing$};
\node[right=.5cm of bob] (bobx) {$\varnothing$};
% \node[right=.5cm of 8a] (8x) {$\varnothing$};
\draw[*->] let \p1 = (carol.two), \p2 = (carol.center) in (\x1,\y2) -- (eve);
\draw[*->] let \p1 = (eve.two), \p2 = (eve.center) in (\x1,\y2) -- (evex);
\draw[*->] let \p1 = (alice.two), \p2 = (alice.center) in (\x1,\y2) -- (dan);
\draw[*->] let \p1 = (dan.two), \p2 = (dan.center) in (\x1,\y2) -- (danx);
...
\draw[->] (t0) edge (carol) (t1) edge (alice) (t2) edge (t2x) (t3) edge (bob);
\end{tikzpicture}
\end{document}
